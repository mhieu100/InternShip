...
const Products = () => {
  ...
  const { products, loading, searchTerm, category, priceRange } =
  ...
  const filteredProducts = products
    .filter((product) => {
      ...
      if (searchTerm) {
        ...
        const nameMatch =
          advancedSearch.inName &&
          product.name?.toLowerCase().includes(searchLower)
        const descMatch =
          advancedSearch.inDescription &&
          product.description?.toLowerCase().includes(searchLower)
        const catMatch =
          advancedSearch.inCategory &&
      ...
      const matchesBrands =
        ...
        selectedBrands.includes(product.brand || product.category)
      const matchesColors =
    ...
    .sort((a, b) => {
      switch (sortBy) {
        ...
        case 'newest':
          return (
            new Date(b.created || Date.now()) -
            new Date(a.created || Date.now())
          )
        case 'popularity':
  ...
  const brands = [
    ...new Set(products.map((p) => p.brand || p.category))
  ].filter(Boolean)
  const colors = [
    ...new Set(products.flatMap((p) => p.colors || [p.color]).filter(Boolean))
  ]
  const sizes = [
    ...new Set(products.flatMap((p) => p.sizes || [p.size]).filter(Boolean))
  ]
      ...
      <div className="max-w-7xl mx-auto p-6">
        ...
        <div className="mb-8 text-center">
          ...
          <Text type="secondary" className="text-lg">
            Discover our amazing collection of {products.length} premium
            products
          </Text>
                  ...
                  <div className="flex flex-wrap gap-2">
                    ...
                    <Checkbox
                      ...
                      onChange={(e) =>
                        setAdvancedSearch((prev) => ({
                          ...prev,
                          inDescription: e.target.checked
                        }))
                      }
                    >
                      Description
                      ...
                      <div className="flex justify-between items-center">
                        <div className="flex items-center gap-2">
                          ...
                          <Badge
                            ...
                            style={{
                              backgroundColor:
                                (product.stock ?? 0) > 0 ? '#52c41a' : '#ff4d4f'
                            }}
                          />
                        </div>
                        <Space>
